...
void print_database(Student_t *p_database, int number_of_students) {
    for (int i = 0; i < number_of_students; i++) {
        int id = p_database[i].id;
        char *name = p_database[i].name;
        char *student_card_number = p_database[i].student_card_number;
        float average_grade = p_database[i].average_grade;
        char *login = p_database[i].login;
        printf("Student with id: %d\nName: %s\nStudent card number: %s\nAverage grade: %.2f\nLogin: %s\n\n",
               id, name, student_card_number, average_grade, login);
    }
    puts("");
}
